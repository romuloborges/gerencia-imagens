...
public class NotificacaoSqliteDAO implements INotificacaoDAO {
    ...
    public List<Notificacao> obterTodasNotificacoes(Long idUsuarioDestinatario) throws Exception {
        ...
        try {
            String SQL = "SELECT n.id, n.mensagem, n.visualizada, n.data_enviada, remetente.id, remetente.nome, destinatario.id, destinatario.nome, i.id, i.caminho FROM Notificacao n INNER JOIN Usuario remetente ON remetente.id = n.enviada_por INNER JOIN Usuario destinatario ON destinatario.id = n.enviada_para INNER JOIN Imagem i ON i.id = n.id_imagem WHERE n.enviada_para = ?;";
    ...
    public Long contaNotificacoesNaoLidas(Long idUsuarioDestinatario) throws Exception {
        ...
        try {
            String SQL = "SELECT COUNT(*) FROM Notificacao n INNER JOIN Usuario destinatario ON destinatario.id = n.enviada_para WHERE destinatario.id = ?;";
